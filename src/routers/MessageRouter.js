...
const MessageRouter = {
    Get: {
        ['/messages']: async ({ headers: { authorization } }, res) => {
            const messages = await MessageService.index({ match: {} }, authorization)
            return res.json(messages)
        },
        ['/messages/:id']: async ({ headers: { authorization } }, res)  => {
    ...
    Delete: {
        ['/messages/:id']: async ({ params, headers: { authorization: token } }, res) => await authVerify(token, 
            async user => {
                const messag = await MessageService.remove({ _id: params.id, user })
                return res.json(messag)
            }
        ),
    }
}
